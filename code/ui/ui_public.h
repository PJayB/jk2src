...
typedef struct {
	...
	int			(*_R_Font_StrLenPixels)(const char *text, const int setIndex, const float scale/* = 1.0f*/);
	int			(*R_Font_StrLenChars)(const char *text);
	int			(*_R_Font_HeightPixels)(const int setIndex, const float scale/* = 1.0f*/);
	void		(*_R_Font_DrawString)(int ox, int oy, const char *text, const float *rgba, const int setIndex, int iMaxPixelWidth, const float scale/* = 1.0f*/);
	qboolean	(*Language_IsAsian) (void);
	qboolean	(*Language_UsesSpaces) (void);
	unsigned int (*AnyLanguage_ReadCharFromString)( const char **ppsText, qboolean *pbIsTrailingPunctuation /* = NULL */);


	inline int		R_Font_HeightPixels(const int index, const float scale = 1.0f)
	{
		return _R_Font_HeightPixels( index, scale );
	}
	inline int		R_Font_StrLenPixels(const char *s, const int index, const float scale = 1.0f)
	{
		return _R_Font_StrLenPixels( s, index, scale );
	}
	inline void	R_Font_DrawString(int x, int y, const char *s, const float *rgba, const int iFontHandle, int iMaxPixelWidth, const float scale = 1.0f)
	{
		return _R_Font_DrawString( x, y, s, rgba, iFontHandle, iMaxPixelWidth, scale );
	}
