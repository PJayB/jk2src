...
typedef struct {
	...
	int		(*_Font_HeightPixels)(const int index, const float scale/* = 1.0f*/);
	int		(*_Font_StrLenPixels)(const char *s, const int index, const float scale/* = 1.0f*/);
	int		(*Font_StrLenChars) (const char *s);
	void	(*_Font_DrawString)(int x, int y, const char *s, const float *rgba, const int iFontHandle, int iMaxPixelWidth, const float scale/* = 1.0f*/);
	qboolean (*Language_IsAsian) (void);
	qboolean (*Language_UsesSpaces) (void);
	unsigned int (*AnyLanguage_ReadCharFromString)( const char **ppsText, qboolean *pbIsTrailingPunctuation /* = NULL */);

	inline int		Font_HeightPixels(const int index, const float scale = 1.0f)
	{
		return _Font_HeightPixels( index, scale );
	}
	inline int		Font_StrLenPixels(const char *s, const int index, const float scale = 1.0f)
	{
		return _Font_StrLenPixels( s, index, scale );
	}
	inline void	Font_DrawString(int x, int y, const char *s, const float *rgba, const int iFontHandle, int iMaxPixelWidth, const float scale = 1.0f)
	{
